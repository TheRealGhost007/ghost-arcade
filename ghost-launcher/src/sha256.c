...
/* A plain, dependency-free SHA-256 (FIPS 180-4). It only has to hash one short
 * string at start-up, so clarity beats speed. */
...
void Sha256_Hex(const void *data, size_t len, char out[65]) {
    ...
    size_t left = len;
    while (left >= 64) { Compress(h, p); p += 64; left -= 64; }

    uint8_t tail[128];
    memset(tail, 0, sizeof(tail));
    if (left) memcpy(tail, p, left);
    tail[left] = 0x80;
    size_t total = left + 1 + 8 <= 64 ? 64 : 128;
    uint64_t bits = (uint64_t)len * 8u;
    for (int i = 0; i < 8; i++) tail[total - 1 - (size_t)i] = (uint8_t)(bits >> (8 * i));
    Compress(h, tail);
