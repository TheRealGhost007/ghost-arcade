...
static bool GetAppDir(char *out, size_t outSize, const char *kind) {
    ...
    /* mkdir -p for base (it usually already exists) and base/brickburst. */
    char cursor[600] = {0};
...
void Settings_Default(Settings *s) {
    ...
    snprintf(s->launch, KEYNAME_LEN, "SPACE");
    snprintf(s->launch2, KEYNAME_LEN, "UP");
...
static void SetField(Settings *s, const char *key, const char *value) {
    ...
    else if (strcmp(key, "launch") == 0) snprintf(s->launch, KEYNAME_LEN, "%s", value);
    else if (strcmp(key, "launch2") == 0) snprintf(s->launch2, KEYNAME_LEN, "%s", value);
...
bool Settings_Save(const Settings *s) {
    ...
    fprintf(f,
        ...
        "audio_enabled=%d\n"
        "mouse_control=%d\n",
        s->left, s->left2, s->right, s->right2, s->launch, s->launch2,
        s->pause, s->restart, s->quit,
        s->volumePercent, s->audioEnabled ? 1 : 0, s->mouseControl ? 1 : 0);
