 ...
 * single mode Rockdrift has on the shared score tables. */
#define GHOST_GAME_NAME "Rockdrift"
#define GHOST_GAME_SLUG "rockdrift"
...
enum {
    SFX_SELECT, SFX_FIRE, SFX_THRUST, SFX_BANG, SFX_BEAT, SFX_SAUCER, SFX_SAUCER_FIRE,
    SFX_DEATH, SFX_HYPER, SFX_EXTRA_LIFE, SFX_GAME_OVER, SFX_COUNT
};
static const char *const kSfxFiles[SFX_COUNT] = {
    "select.wav", "fire.wav", "thrust.wav", "bang.wav", "beat.wav", "saucer.wav", "saucerfire.wav",
    "death.wav", "hyper.wav", "extralife.wav", "gameover.wav",
};

static const char *const kNotes_1_0_0[] = {
    "Initial release: a vector-line space-rock shooter, in glowing lines with no filled shapes at all",
    "Real inertia: the ship keeps drifting until you turn round and thrust the other way",
    "The field wraps for the ship, the rocks and every bullet",
    "Large rocks split into two medium, medium into two small; the smaller the rock, the more it is worth",
    "Two saucers: the big one fires anywhere, the small one aims, and aims better the higher your score",
    "Hyperspace throws you somewhere random, with a one-in-six chance of coming out in pieces",
    "A heartbeat that speeds up as the rocks run out, and an extra ship every 10,000 points",
    "Optional glow; shares its score table and your name with Ghost Launcher",
    NULL,
};
...
typedef struct {
    int left, left2, right, right2, thrust, thrust2, fire, hyper, pause, restart, quit;
} KeyMap;
...
static void BuildKeyMap(const Settings *s, KeyMap *out) {
    ...
    out->thrust = Keys_CodeFromName(s->thrust);
    out->thrust2 = Keys_CodeFromName(s->thrust2);
    out->fire = Keys_CodeFromName(s->fire);
    out->hyper = Keys_CodeFromName(s->hyper);
...
static char *SettingsRowField(Settings *s, int row) {
    switch (row) {
        ...
        case 4: return s->thrust;
        case 5: return s->thrust2;
        case 6: return s->fire;
        case 7: return s->hyper;
...
static void BuildSettingsRows(Settings *s, UiSettingsRow rows[SETTINGS_ROW_COUNT]) {
    static const char *kLabels[SETTINGS_REBIND_ROWS] = {
        "Turn left", "Turn left, second key", "Turn right", "Turn right, second key",
        "Thrust", "Thrust, second key", "Fire", "Hyperspace", "Pause", "Restart", "Back to menu",
    };
    for (int i = 0; i < SETTINGS_REBIND_ROWS; i++) {
        rows[i] = (UiSettingsRow){UI_ROW_KEY, kLabels[i], SettingsRowField(s, i), false, 0};
    }
    rows[SETTINGS_ROW_GLOW] = (UiSettingsRow){UI_ROW_TOGGLE, "Glow", NULL, s->glow, 0};
...
static void ResolveAssetsDir(char *buf, size_t bufSize) {
    ...
    if (xdgData && xdgData[0] != '\0') {
        snprintf(buf, bufSize, "%s/rockdrift/assets", xdgData);
    } else {
        const char *home = getenv("HOME");
        snprintf(buf, bufSize, "%s/.local/share/rockdrift/assets", home ? home : "");
    }
    if (DirectoryExists(buf)) return;
...
int main(int argc, char **argv) {
    ...
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Rockdrift");
    SetExitKey(KEY_NULL); /* we handle Escape ourselves */
    SetTargetFPS(60);

    char assetsDir[512], iconPath[600];
    ResolveAssetsDir(assetsDir, sizeof(assetsDir));
    snprintf(iconPath, sizeof(iconPath), "%s/icons/rockdrift.png", assetsDir);

    UiTheme theme = Ui_DefaultTheme((Color){180, 255, 100, 255});
    ...
    int restartNonce = 1;
    bool showFps = false;
    float engineTimer = 0.0f, saucerTimer = 0.0f;
    ...
    while (!WindowShouldClose()) {
        ...
        if (appState == APP_MENU) {
            ...
            UiMenu menu = {
                .title = "ROCKDRIFT", .tagline = "Nothing out here stays in one piece.",
                ...
                .iconPath = iconPath, .versionLabel = Daily_Active() ? "v" ROCKDRIFT_VERSION "  DAILY" : "v" ROCKDRIFT_VERSION, .backdrop = Render_MenuBackdrop,
            };
            Ui_DrawMenu(&menu, dt);
            continue;
        }
            ...
            if (awaitingKey) {
                ...
                if (settingsRow >= SETTINGS_ROW_COUNT - 1 && settingsRow < SETTINGS_ROW_COUNT - 1 + PREFS_ROWS) {
                    ...
                    if (settingsRow < SETTINGS_REBIND_ROWS) {
                        awaitingKey = true;
                    } else if (settingsRow == SETTINGS_ROW_GLOW) {
        ...
        float turn = (leftHeld == rightHeld) ? 0.0f : (leftHeld ? -1.0f : 1.0f);
        Game_SetInput(&game, turn, IsKeyDown(keys.thrust) || IsKeyDown(keys.thrust2), IsKeyDown(keys.fire), IsKeyDown(keys.hyper));

        Game_Update(&game, dt * Tune_Speed());

        if (game.justFired) Sfx_Play(SFX_FIRE);
        if (game.justBeat) Sfx_PlayPitched(SFX_BEAT, game.beatNote ? 1.0f : 0.8f);
        if (game.justSaucerFired) Sfx_Play(SFX_SAUCER_FIRE);
        if (game.justHyperspace) Sfx_Play(SFX_HYPER);
        if (game.justExtraLife) Sfx_Play(SFX_EXTRA_LIFE);
        for (int i = 0; i < game.killCount; i++) {
            switch (game.kills[i].kind) {
                case KILL_ROCK_LARGE: Sfx_PlayPitched(SFX_BANG, 0.75f); break;
                case KILL_ROCK_MEDIUM: Sfx_PlayPitched(SFX_BANG, 1.0f); break;
                case KILL_ROCK_SMALL: Sfx_PlayPitched(SFX_BANG, 1.4f); break;
                case KILL_SAUCER_LARGE: Sfx_PlayPitched(SFX_BANG, 0.6f); break;
                case KILL_SAUCER_SMALL: Sfx_PlayPitched(SFX_BANG, 0.9f); break;
                case KILL_SHIP: Sfx_Play(SFX_DEATH); break;
            }
        }
        /* Engine rumble and saucer warble are short samples retriggered on a
         * timer while they last, which needs no looping support. */
        engineTimer -= dt;
        if (game.ship.thrusting && engineTimer <= 0.0f) { Sfx_Play(SFX_THRUST); engineTimer = 0.11f; }
        saucerTimer -= dt;
        if (game.saucer.active && saucerTimer <= 0.0f) { Sfx_PlayPitched(SFX_SAUCER, game.saucer.small ? 1.35f : 1.0f); saucerTimer = 0.36f; }
        ...
        FrameInfo info = {username, lastRank, settings.glow, showFps};
