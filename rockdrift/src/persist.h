...
typedef struct {
    ...
    char thrust[KEYNAME_LEN];
    char thrust2[KEYNAME_LEN];
    char fire[KEYNAME_LEN];
    char hyper[KEYNAME_LEN];
    char pause[KEYNAME_LEN];
    char restart[KEYNAME_LEN];
    char quit[KEYNAME_LEN];
    int volumePercent; /* 0-100 */
    bool audioEnabled;
    bool glow;         /* the additive glow around every line */
} Settings;
...
/* Loads from $XDG_CONFIG_HOME/rockdrift/config.ini, falling back to
...
/* Loads from $XDG_DATA_HOME/rockdrift/save.dat. Returns true if a file
 * was found and read. */
