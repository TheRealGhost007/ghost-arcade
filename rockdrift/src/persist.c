...
static bool GetAppDir(char *out, size_t outSize, const char *kind) {
    ...
    /* mkdir -p for base (it usually already exists) and base/rockdrift. */
    char cursor[600] = {0};
...
void Settings_Default(Settings *s) {
    ...
    snprintf(s->hyper, KEYNAME_LEN, "LEFT_SHIFT");
...
static void SetField(Settings *s, const char *key, const char *value) {
    ...
    else if (strcmp(key, "hyper") == 0) snprintf(s->hyper, KEYNAME_LEN, "%s", value);
...
bool Settings_Save(const Settings *s) {
    ...
    fprintf(f,
        ...
        "audio_enabled=%d\n"
        "glow=%d\n",
        s->left, s->left2, s->right, s->right2, s->thrust, s->thrust2, s->fire, s->hyper,
