 ...
 * single mode Crawlshot has on the shared score tables. */
#define GHOST_GAME_NAME "Crawlshot"
#define GHOST_GAME_SLUG "crawlshot"
...
enum {
    SFX_SELECT, SFX_SHOOT, SFX_MUSH_HIT, SFX_MUSH_BREAK, SFX_HIT, SFX_CREATURE_KILL, SFX_SPIDER, SFX_FLEA,
    SFX_SCORPION, SFX_DEATH, SFX_RESTORE, SFX_WAVE_CLEAR, SFX_EXTRA_LIFE, SFX_GAME_OVER, SFX_COUNT
};
static const char *const kSfxFiles[SFX_COUNT] = {
    "select.wav", "shoot.wav", "mushhit.wav", "mushbreak.wav", "hit.wav", "creaturekill.wav", "spider.wav", "flea.wav",
    "scorpion.wav", "death.wav", "restore.wav", "waveclear.wav", "extralife.wav", "gameover.wav",
};

static const char *const kNotes_1_0_0[] = {
    "Initial release: a twelve-segment caterpillar comes down a garden of toadstools and you shoot it from the bottom rows",
    "Shoot a segment and it becomes a toadstool; the caterpillar splits, and the back half grows a new head",
    "Toadstools take four shots; caterpillars drop a row and turn round at every one, and climb back up if they reach the bottom",
    "The spider zig-zags through your garden eating toadstools: shoot it close for 900, far away for 300",
    "The flea drops down planting toadstools when your garden is nearly bare, and takes two shots",
    "The scorpion poisons a row of toadstools; a caterpillar that touches one dives straight to the bottom",
    "When you lose a shooter, damaged toadstools are healed one by one for 5 points each",
    "Each wave splits the twelve segments into a shorter caterpillar and more single heads, and everything moves faster",
    "An extra shooter every 12,000 points; optional mouse steering and CRT scanlines; shares its score table and your name with Ghost Launcher",
    NULL,
};
...
typedef struct {
    int up, up2, down, down2, left, left2, right, right2, fire, pause, restart, quit;
} KeyMap;
...
#define SETTINGS_REBIND_ROWS 12
#define SETTINGS_ROW_SCANLINES 12
#define SETTINGS_ROW_MOUSE 13
#define SETTINGS_ROW_VOLUME 14
#define SETTINGS_ROW_AUDIO 15
#define SETTINGS_ROW_COUNT 17
...
static char *SettingsRowField(Settings *s, int row) {
    switch (row) {
        ...
        case 8: return s->fire;
        case 9: return s->pause;
        case 10: return s->restart;
        case 11: return s->quit;
        default: return NULL;
    }
}
...
static void BuildSettingsRows(Settings *s, UiSettingsRow rows[SETTINGS_ROW_COUNT]) {
    static const char *kLabels[SETTINGS_REBIND_ROWS] = {
        ...
        "Right", "Right, second key", "Fire", "Pause", "Restart", "Back to menu",
    };
    ...
    rows[SETTINGS_ROW_MOUSE] = (UiSettingsRow){UI_ROW_TOGGLE, "Mouse steering", NULL, s->mouseControl, 0};
...
static void ResolveAssetsDir(char *buf, size_t bufSize) {
    ...
    if (xdgData && xdgData[0] != '\0') {
        snprintf(buf, bufSize, "%s/crawlshot/assets", xdgData);
    } else {
        const char *home = getenv("HOME");
        snprintf(buf, bufSize, "%s/.local/share/crawlshot/assets", home ? home : "");
    }
    if (DirectoryExists(buf)) return;
...
int main(int argc, char **argv) {
    ...
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Crawlshot");
    SetExitKey(KEY_NULL); /* we handle Escape ourselves */
    SetTargetFPS(60);

    char assetsDir[512], iconPath[600];
    ResolveAssetsDir(assetsDir, sizeof(assetsDir));
    snprintf(iconPath, sizeof(iconPath), "%s/icons/crawlshot.png", assetsDir);

    UiTheme theme = Ui_DefaultTheme((Color){232, 104, 255, 255});
    ...
    while (!WindowShouldClose()) {
        ...
        if (appState == APP_MENU) {
            ...
            UiMenu menu = {
                .title = "CRAWLSHOT", .tagline = "Twelve segments. One garden. One shot at a time.",
                ...
                .iconPath = iconPath, .versionLabel = Daily_Active() ? "v" CRAWLSHOT_VERSION "  DAILY" : "v" CRAWLSHOT_VERSION, .backdrop = Render_MenuBackdrop,
            };
            Ui_DrawMenu(&menu, dt);
            continue;
        }
...

        {
            ...
            if (IsKeyDown(keys.left) || IsKeyDown(keys.left2)) mx -= 1.0f;
            if (IsKeyDown(keys.right) || IsKeyDown(keys.right2)) mx += 1.0f;
            if (IsKeyDown(keys.up) || IsKeyDown(keys.up2)) my -= 1.0f;
            if (IsKeyDown(keys.down) || IsKeyDown(keys.down2)) my += 1.0f;
            bool fire = IsKeyDown(keys.fire);
            if (settings.mouseControl) {
                /* Steer toward the pointer; the shooter stops once it is there. */
                Vector2 m = GetMousePosition();
                float dx = Render_ScreenToFieldX(m.x) - game.px, dy = Render_ScreenToFieldY(m.y) - game.py;
                if (fabsf(dx) > 0.15f) mx += dx * 4.0f;
                if (fabsf(dy) > 0.15f) my += dy * 4.0f;
                if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) fire = true;
            }
            Game_SetInput(&game, mx, my, fire);
        }

        Game_Update(&game, dt * Tune_Speed());

        if (game.justFired) Sfx_PlayPitched(SFX_SHOOT, 0.92f + 0.16f * (float)(game.score % 7) / 7.0f);
        if (game.justMushroomHit) Sfx_Play(SFX_MUSH_HIT);
        for (int i = 0; i < game.killCount; i++) {
            switch (game.kills[i].kind) {
                case KILL_HEAD: case KILL_BODY: Sfx_Play(SFX_HIT); break;
                case KILL_MUSHROOM: Sfx_Play(SFX_MUSH_BREAK); break;
                default: Sfx_Play(SFX_CREATURE_KILL); break;
            }
        }
        if (game.justSpiderAppeared) Sfx_Play(SFX_SPIDER);
        if (game.justFleaAppeared) Sfx_Play(SFX_FLEA);
        if (game.justScorpionAppeared) Sfx_Play(SFX_SCORPION);
        if (game.justPlayerDeath) Sfx_Play(SFX_DEATH);
        if (game.justRestoreTick) Sfx_PlayPitched(SFX_RESTORE, 1.0f + 0.02f * (float)(game.score % 10));
        if (game.justExtraLife) Sfx_Play(SFX_EXTRA_LIFE);
        if (game.justPowerUp) Sfx_Play(SFX_CREATURE_KILL);
