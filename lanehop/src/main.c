 ...
 * single mode Lanehop has on the shared score tables. */
#define GHOST_GAME_NAME "Lanehop"
#define GHOST_GAME_SLUG "lanehop"
...
enum {
    SFX_SELECT, SFX_HOP, SFX_SPLAT, SFX_SPLASH, SFX_HOME, SFX_FLY, SFX_TICK,
    SFX_LEVEL_CLEAR, SFX_EXTRA_LIFE, SFX_GAME_OVER, SFX_COUNT
};
static const char *const kSfxFiles[SFX_COUNT] = {
    "select.wav", "hop.wav", "splat.wav", "splash.wav", "home.wav", "fly.wav", "tick.wav",
    "levelclear.wav", "extralife.wav", "gameover.wav",
};

static const char *const kNotes_1_0_0[] = {
    "Initial release: a hare crosses five lanes of traffic and five of river to reach one of five burrows",
    "Logs and turtles carry you; open water, a car, the hedge, a taken burrow or the clock all end a hare",
    "Some turtles dive: they wobble first, then go under, each group on its own beat",
    "A fly in a burrow is worth 200; a crocodile in one is a trap",
    "Quick taps chain into a run of hops; 30 seconds per hare, 10 points a new row, and time left counts when you get home",
    "Fill all five burrows to clear the level: a 1000 bonus, and everything runs faster next time",
    "An extra hare every 10,000 points; optional CRT scanlines; shares its score table and your name with Ghost Launcher",
    NULL,
};
...
static void ResolveAssetsDir(char *buf, size_t bufSize) {
    ...
    if (xdgData && xdgData[0] != '\0') {
        snprintf(buf, bufSize, "%s/lanehop/assets", xdgData);
    } else {
        const char *home = getenv("HOME");
        snprintf(buf, bufSize, "%s/.local/share/lanehop/assets", home ? home : "");
    }
    if (DirectoryExists(buf)) return;
...
int main(int argc, char **argv) {
    ...
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Lanehop");
    SetExitKey(KEY_NULL); /* we handle Escape ourselves */
    SetTargetFPS(60);

    char assetsDir[512], iconPath[600];
    ResolveAssetsDir(assetsDir, sizeof(assetsDir));
    snprintf(iconPath, sizeof(iconPath), "%s/icons/lanehop.png", assetsDir);

    UiTheme theme = Ui_DefaultTheme((Color){255, 104, 96, 255});
    ...
    while (!WindowShouldClose()) {
        ...
        if (appState == APP_MENU) {
            ...
            UiMenu menu = {
                .title = "LANEHOP", .tagline = "Five lanes of traffic. Five of river. One hare.",
                ...
                .iconPath = iconPath, .versionLabel = Daily_Active() ? "v" LANEHOP_VERSION "  DAILY" : "v" LANEHOP_VERSION, .backdrop = Render_MenuBackdrop,
            };
            Ui_DrawMenu(&menu, dt);
            continue;
        }
        ...
        /* A hop per key press; the game buffers one more while a hop is in flight. */
        if (IsKeyPressed(keys.up) || IsKeyPressed(keys.up2)) Game_Hop(&game, DIR_UP);
        if (IsKeyPressed(keys.down) || IsKeyPressed(keys.down2)) Game_Hop(&game, DIR_DOWN);
        if (IsKeyPressed(keys.left) || IsKeyPressed(keys.left2)) Game_Hop(&game, DIR_LEFT);
        if (IsKeyPressed(keys.right) || IsKeyPressed(keys.right2)) Game_Hop(&game, DIR_RIGHT);

        Game_Update(&game, dt * Tune_Speed());

        if (game.justHop) Sfx_Play(SFX_HOP);
        if (game.justTimeWarning) Sfx_Play(SFX_TICK);
        if (game.justHome) Sfx_Play(SFX_HOME);
        if (game.justFly) Sfx_Play(SFX_FLY);
        if (game.justGoodie) Sfx_Play(SFX_FLY);
        if (game.justShieldSave) Sfx_Play(SFX_SPLASH);
        if (game.justExtraLife) Sfx_Play(SFX_EXTRA_LIFE);
        if (game.justLevelClear) Sfx_Play(SFX_LEVEL_CLEAR);
        if (game.justDeath) Sfx_Play(game.deathCause == DEATH_CAR ? SFX_SPLAT : SFX_SPLASH);
