 ...
 * single mode Moondrop has on the shared score tables. */
#define GHOST_GAME_NAME "Moondrop"
#define GHOST_GAME_SLUG "moondrop"
...
enum {
    SFX_SELECT, SFX_THRUST, SFX_TOUCH, SFX_LAND, SFX_CRASH, SFX_LOW_FUEL, SFX_NEW_LEVEL, SFX_GAME_OVER, SFX_COUNT
};
static const char *const kSfxFiles[SFX_COUNT] = {
    "select.wav", "thrust.wav", "touch.wav", "land.wav", "crash.wav", "lowfuel.wav", "newlevel.wav", "gameover.wav",
};

static const char *const kNotes_1_0_0[] = {
    "Initial release: land a lander on procedurally generated moon terrain, in glowing vector lines",
    "Real physics: gravity, thrust along the nose, and fuel that only burns while the engine does",
    "Land on a flat pad with both feet, under the speed limits and within 6 degrees of upright",
    "Narrow pads pay more: x1, x2, x3 and x5, plus a tenth of your fuel and a bonus for a feather-light touch",
    "Each landing refuels a little and the next moon is rougher, with fewer and narrower pads",
    "A crash costs fuel, not a life; the run ends when the tank is empty",
    "The camera zooms in as you near the ground; readouts turn red when a number would be too much to land on",
...
typedef struct {
    int left, left2, right, right2, thrust, thrust2, pause, restart, quit;
} KeyMap;
...
static void BuildSettingsRows(Settings *s, UiSettingsRow rows[SETTINGS_ROW_COUNT]) {
    static const char *kLabels[SETTINGS_REBIND_ROWS] = {
        "Rotate left", "Rotate left, second key", "Rotate right", "Rotate right, second key",
        "Thrust", "Thrust, second key", "Pause", "Restart", "Back to menu",
    };
...
static void ResolveAssetsDir(char *buf, size_t bufSize) {
    ...
    if (xdgData && xdgData[0] != '\0') {
        snprintf(buf, bufSize, "%s/moondrop/assets", xdgData);
    } else {
        const char *home = getenv("HOME");
        snprintf(buf, bufSize, "%s/.local/share/moondrop/assets", home ? home : "");
    }
    if (DirectoryExists(buf)) return;
...
int main(int argc, char **argv) {
    ...
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Moondrop");
    SetExitKey(KEY_NULL); /* we handle Escape ourselves */
    SetTargetFPS(60);

    char assetsDir[512], iconPath[600];
    ResolveAssetsDir(assetsDir, sizeof(assetsDir));
    snprintf(iconPath, sizeof(iconPath), "%s/icons/moondrop.png", assetsDir);

    UiTheme theme = Ui_DefaultTheme((Color){255, 184, 64, 255});
    Ui_Init(assetsDir, &theme, WINDOW_WIDTH, WINDOW_HEIGHT);
    Win_Init(WINDOW_WIDTH, WINDOW_HEIGHT);
    Tune_Init(GHOST_GAME_SLUG);
    Tune_UiInit();
    Game_SetTuning(Tune_Add("gravity", "Gravity", GRAVITY, 10.0f, 40.0f, 1.0f), Tune_Add("thrust", "Engine thrust", THRUST, 30.0f, 100.0f, 2.0f),
                   Tune_Add("burn", "Fuel burn", FUEL_BURN, 10.0f, 60.0f, 2.0f));
    ...
    while (!WindowShouldClose()) {
        ...
        if (appState == APP_MENU) {
            ...
            UiMenu menu = {
                .title = "MOONDROP", .tagline = "Gentle. Level. On the pad. Not much fuel.",
                ...
                .iconPath = iconPath, .versionLabel = Daily_Active() ? "v" MOONDROP_VERSION "  DAILY" : "v" MOONDROP_VERSION, .backdrop = Render_MenuBackdrop,
            };
            Ui_DrawMenu(&menu, dt);
            continue;
        }
        ...
        Game_SetInput(&game, turn, IsKeyDown(keys.thrust) || IsKeyDown(keys.thrust2));

        Game_Update(&game, dt * Tune_Speed());

        if (game.justLand) Sfx_Play(SFX_LAND);
        if (game.justPod) Sfx_Play(SFX_NEW_LEVEL);
        if (game.justCrash) Sfx_Play(SFX_CRASH);
        if (game.justLowFuel) Sfx_Play(SFX_LOW_FUEL);
        if (game.justNewLevel || game.justRespawn) Sfx_Play(SFX_NEW_LEVEL);
        /* The engine is a short sample retriggered on a timer while it burns,
         * which needs no looping support. */
        engineTimer -= dt;
        if (game.thrusting && engineTimer <= 0.0f) { Sfx_Play(SFX_THRUST); engineTimer = 0.11f; }
