 ...
 * single mode Ghostmaze has on the shared score tables. */
#define GHOST_GAME_NAME "Ghostmaze"
#define GHOST_GAME_SLUG "ghostmaze"
#define GHOST_GAME_MODE "arcade"

typedef enum { APP_MENU, APP_PLAYING, APP_SCORES, APP_SETTINGS, APP_UPDATES, APP_HOWTO } AppState;

enum {
    SFX_SELECT, SFX_POSSESS, SFX_RELEASE, SFX_STEP, SFX_BLOCKED, SFX_PUSH, SFX_SMASH, SFX_KEY, SFX_DOOR,
    SFX_GATE, SFX_BELL, SFX_CAUGHT, SFX_CLEAR, SFX_EXTRA_LIFE, SFX_GAME_OVER, SFX_COUNT
};
static const char *const kSfxFiles[SFX_COUNT] = {
    "select.wav", "possess.wav", "release.wav", "step.wav", "blocked.wav", "push.wav", "smash.wav", "key.wav", "door.wav",
    "gate.wav", "bell.wav", "caught.wav", "clear.wav", "extralife.wav", "gameover.wav",
};

static const char *const kNotes_2_0_0[] = {
    "Rebuilt from the ground up: no more dots or pellets. You are a ghost in a haunted house, and a priest with a bell walks the halls",
    "The priest's sight destroys a ghost, and you can see exactly what he sees: the yellow beam along his facing",
    "Possess things: a cat squeezes through cat flaps, a suit of armor is slow but breaks cracked walls and pushes crates, a servant carries keys and opens doors",
    "Only a ghost can use the exit mirror, so you have to leave your host at the end",
    "Pressure plates open gates while something is standing on them; a host or a crate will do, a ghost is too light",
    "Fifteen rooms, each proven solvable by a solver that also sets the par; the house goes round again after the last",
...
typedef struct {
    int up, up2, down, down2, left, left2, right, right2, possess, pause, restart, quit;
} KeyMap;
...
static void BuildKeyMap(const Settings *s, KeyMap *out) {
    ...
    out->possess = Keys_CodeFromName(s->possess);
...
static void BuildSettingsRows(Settings *s, UiSettingsRow rows[SETTINGS_ROW_COUNT]) {
    static const char *kLabels[SETTINGS_REBIND_ROWS] = {
        ...
        "Right", "Right, second key", "Possess or leave", "Pause", "Restart", "Back to menu",
    };
...
static void ResolveAssetsDir(char *buf, size_t bufSize) {
    ...
    if (xdgData && xdgData[0] != '\0') {
        snprintf(buf, bufSize, "%s/ghostmaze/assets", xdgData);
    } else {
        const char *home = getenv("HOME");
        snprintf(buf, bufSize, "%s/.local/share/ghostmaze/assets", home ? home : "");
    }
    if (DirectoryExists(buf)) return;
...
int main(void) {
    ...
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Ghostmaze");
    SetExitKey(KEY_NULL); /* we handle Escape ourselves */
    SetTargetFPS(60);

    char assetsDir[512], iconPath[600];
    ResolveAssetsDir(assetsDir, sizeof(assetsDir));
    snprintf(iconPath, sizeof(iconPath), "%s/icons/ghostmaze.png", assetsDir);

    UiTheme theme = Ui_DefaultTheme((Color){255, 110, 190, 255});
    ...
    Game game;
    Game_Init(&game, saveData.highScore);
    ...
    while (!WindowShouldClose()) {
        ...
        if (appState == APP_MENU) {
            ...
            char statLine[96];
            snprintf(statLine, sizeof(statLine), "Best  %ld      furthest  room %d", saveData.highScore, saveData.bestLevel);
            UiMenu menu = {
                .title = "GHOSTMAZE", .tagline = "You are the ghost. He has a bell.",
                ...
                .iconPath = iconPath, .versionLabel = "v" GHOSTMAZE_VERSION, .backdrop = Render_MenuBackdrop,
            };
            Ui_DrawMenu(&menu, dt);
            continue;
        }
        ...
        if (IsKeyPressed(keys.up) || IsKeyPressed(keys.up2)) Game_Queue(&game, ACT_UP);
        if (IsKeyPressed(keys.down) || IsKeyPressed(keys.down2)) Game_Queue(&game, ACT_DOWN);
        if (IsKeyPressed(keys.left) || IsKeyPressed(keys.left2)) Game_Queue(&game, ACT_LEFT);
        if (IsKeyPressed(keys.right) || IsKeyPressed(keys.right2)) Game_Queue(&game, ACT_RIGHT);
        if (IsKeyPressed(keys.possess)) Game_Queue(&game, ACT_POSSESS);
        /* Holding a direction keeps you moving: refresh the latch each frame. */
        if (game.queued == ACT_WAIT) {
            if (IsKeyDown(keys.up) || IsKeyDown(keys.up2)) Game_Queue(&game, ACT_UP);
            else if (IsKeyDown(keys.down) || IsKeyDown(keys.down2)) Game_Queue(&game, ACT_DOWN);
            else if (IsKeyDown(keys.left) || IsKeyDown(keys.left2)) Game_Queue(&game, ACT_LEFT);
            else if (IsKeyDown(keys.right) || IsKeyDown(keys.right2)) Game_Queue(&game, ACT_RIGHT);
        }

        Game_Update(&game, dt * Tune_Speed());

        if (game.justPossess) Sfx_Play(SFX_POSSESS);
        if (game.justRelease) Sfx_Play(SFX_RELEASE);
        if (game.justStep) Sfx_PlayVaried(SFX_STEP, 0.08f);
        if (game.justBlocked) Sfx_Play(SFX_BLOCKED);
        if (game.justPush) Sfx_Play(SFX_PUSH);
        if (game.justSmash) Sfx_Play(SFX_SMASH);
        if (game.justKey) Sfx_Play(SFX_KEY);
        if (game.justDoor) Sfx_Play(SFX_DOOR);
        if (game.justGate) Sfx_Play(SFX_GATE);
        if (game.justBell) Sfx_PlayVaried(SFX_BELL, 0.05f);
        if (game.justCaught) Sfx_Play(SFX_CAUGHT);
