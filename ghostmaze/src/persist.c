...
static bool GetAppDir(char *out, size_t outSize, const char *kind) {
    ...
    /* mkdir -p for base (it usually already exists) and base/ghostmaze. */
    char cursor[600] = {0};
...
void Settings_Default(Settings *s) {
    ...
    snprintf(s->possess, KEYNAME_LEN, "SPACE");
...
static void SetField(Settings *s, const char *key, const char *value) {
    ...
    else if (strcmp(key, "possess") == 0) snprintf(s->possess, KEYNAME_LEN, "%s", value);
...
bool Settings_Save(const Settings *s) {
    ...
    fprintf(f,
        ...
        s->possess, s->pause, s->restart, s->quit,
