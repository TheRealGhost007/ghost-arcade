...
static bool GetAppDir(char *out, size_t outSize, const char *kind) {
    ...
    /* mkdir -p for base (it usually already exists) and base/coilrush. */
    char cursor[600] = {0};
...
static void SetField(Settings *s, const char *key, const char *value) {
    if (strcmp(key, "up") == 0) snprintf(s->up, KEYNAME_LEN, "%s", value);
    ...
    else if (strcmp(key, "left") == 0) snprintf(s->left, KEYNAME_LEN, "%s", value);
        ...
        for (int m = 0; m < MODE_COUNT; m++) {
            if (strcmp(value, Game_ModeKey((GameMode)m)) == 0) s->mode = (GameMode)m;
        }
    }
}
...
bool Settings_Save(const Settings *s) {
    ...
    GameMode mode = (s->mode >= 0 && s->mode < MODE_COUNT) ? s->mode : MODE_CLASSIC;
    fprintf(f,
        "# coilrush settings\n"
        "# Key names: A-Z, 0-9, LEFT, RIGHT, UP, DOWN, SPACE, ESCAPE, ENTER,\n"
        "# LEFT_SHIFT, RIGHT_SHIFT, LEFT_CONTROL, TAB, etc. Each direction may\n"
        ...
        "audio_enabled=%d\n"
        "mode=%s\n",
        s->up, s->up2, s->down, s->down2, s->left, s->left2, s->right, s->right2,
        s->pause, s->restart, s->quit,
        s->volumePercent, s->audioEnabled ? 1 : 0, Game_ModeKey(mode));
...
bool SaveData_Load(SaveData *d) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        for (int m = 0; m < MODE_COUNT; m++) {
            char expect[48];
            snprintf(expect, sizeof(expect), "high_score_%s", Game_ModeKey((GameMode)m));
            if (strcmp(key, expect) == 0) d->highScore[m] = atol(value);
            snprintf(expect, sizeof(expect), "best_length_%s", Game_ModeKey((GameMode)m));
            if (strcmp(key, expect) == 0) d->bestLength[m] = atoi(value);
        }
    }
    fclose(f);
    return true;
...
bool SaveData_Save(const SaveData *d) {
    ...
    for (int m = 0; m < MODE_COUNT; m++) {
        fprintf(f, "high_score_%s=%ld\n", Game_ModeKey((GameMode)m), d->highScore[m]);
        fprintf(f, "best_length_%s=%d\n", Game_ModeKey((GameMode)m), d->bestLength[m]);
    }
