 ...
 * single mode Girderclimb has on the shared score tables. */
#define GHOST_GAME_NAME "Girderclimb"
#define GHOST_GAME_SLUG "girderclimb"
...
enum {
    SFX_SELECT, SFX_FIRE, SFX_ATTACH, SFX_RELEASE, SFX_JUMP, SFX_LAND, SFX_RING, SFX_CHECKPOINT, SFX_WALL,
    SFX_DEATH, SFX_CLEAR, SFX_EXTRA_LIFE, SFX_GAME_OVER, SFX_COUNT
};
static const char *const kSfxFiles[SFX_COUNT] = {
    "select.wav", "fire.wav", "attach.wav", "release.wav", "jump.wav", "land.wav", "ring.wav", "checkpoint.wav", "wall.wav",
    "death.wav", "clear.wav", "extralife.wav", "gameover.wav",
};

static const char *const kNotes_2_0_0[] = {
    "Rebuilt from the ground up: no more barrels, ape or rivet screens. You climb a burning tower with a grapple hook",
    "Fire at the ring above you, hold Up to reel in, pump your swing with Left and Right, and let go to fly to the next ring",
    "The rope is a real constraint: it turns a fall into a swing, and letting go keeps your speed",
    "Cursed fire rises from the bottom and gets quicker the longer you take; from tower two, ghost orbs drift across the shaft",
    "Rest ledges are checkpoints: lose a life and you start again from the last one you touched",
    "The ring you would grab is marked with a reticle; leaning left or right changes which one",
    "Every tower is generated from a seed, and the tests climb hundreds of them with a simple bot to prove each can be finished",
...
typedef struct {
    int up, up2, down, down2, left, left2, right, right2, grapple, jump, pause, restart, quit;
} KeyMap;
...
static void BuildKeyMap(const Settings *s, KeyMap *out) {
    ...
    out->grapple = Keys_CodeFromName(s->grapple);
    out->jump = Keys_CodeFromName(s->jump);
...
#define SETTINGS_ROW_SCANLINES 13
...
static char *SettingsRowField(Settings *s, int row) {
    switch (row) {
        ...
        case 8: return s->grapple;
        case 9: return s->jump;
...
static void BuildSettingsRows(Settings *s, UiSettingsRow rows[SETTINGS_ROW_COUNT]) {
    static const char *kLabels[SETTINGS_REBIND_ROWS] = {
        ...
        "Right", "Right, second key", "Grapple", "Jump", "Pause", "Restart", "Back to menu",
    };
...
static void ResolveAssetsDir(char *buf, size_t bufSize) {
    ...
    if (xdgData && xdgData[0] != '\0') {
        snprintf(buf, bufSize, "%s/girderclimb/assets", xdgData);
    } else {
        const char *home = getenv("HOME");
        snprintf(buf, bufSize, "%s/.local/share/girderclimb/assets", home ? home : "");
    }
    if (DirectoryExists(buf)) return;
...
int main(int argc, char **argv) {
    ...
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Girderclimb");
    SetExitKey(KEY_NULL); /* we handle Escape ourselves */
    SetTargetFPS(60);

    char assetsDir[512], iconPath[600];
    ResolveAssetsDir(assetsDir, sizeof(assetsDir));
    snprintf(iconPath, sizeof(iconPath), "%s/icons/girderclimb.png", assetsDir);

    UiTheme theme = Ui_DefaultTheme((Color){255, 92, 122, 255});
    ...
    while (!WindowShouldClose()) {
        ...
        if (appState == APP_MENU) {
            ...
            char statLine[96];
            snprintf(statLine, sizeof(statLine), "Best  %ld      furthest  tower %d", saveData.highScore, saveData.bestLevel);
            UiMenu menu = {
                .title = "GIRDERCLIMB", .tagline = "Swing. Reel. Let go. Do not stop.",
                ...
                .iconPath = iconPath, .versionLabel = Daily_Active() ? "v" GIRDERCLIMB_VERSION "  DAILY" : "v" GIRDERCLIMB_VERSION, .backdrop = Render_MenuBackdrop,
            };
            Ui_DrawMenu(&menu, dt);
            continue;
        }
        ...
        if (IsKeyPressed(keys.restart)) {
            FinalizeRun(&game, &saveData, username, &runFinalized);
            restartNonce++;
            Game_Restart(&game, MakeSeed(restartNonce), saveData.highScore);
            Ui_ClearParticles();
            runFinalized = false;
            lastRank = 0;
...

        {
            ...
            Game_SetInput(&game, dx, dy, IsKeyDown(keys.grapple), IsKeyDown(keys.jump));
        }

        Game_Update(&game, dt * Tune_Speed());

        if (game.justFire && !game.justAttach) Sfx_Play(SFX_SELECT);
        if (game.justAttach) Sfx_Play(SFX_ATTACH);
        if (game.justRelease) Sfx_Play(SFX_RELEASE);
        if (game.justJump) Sfx_Play(SFX_JUMP);
        if (game.justLand) Sfx_Play(SFX_LAND);
        if (game.justRing) Sfx_PlayVaried(SFX_RING, 0.05f);
        if (game.justCheckpoint) Sfx_Play(SFX_CHECKPOINT);
        if (game.justWall) Sfx_Play(SFX_WALL);
