...
static bool GetAppDir(char *out, size_t outSize, const char *kind) {
    ...
    /* mkdir -p for base (it usually already exists) and base/girderclimb. */
    char cursor[600] = {0};
...
void Settings_Default(Settings *s) {
    ...
    snprintf(s->grapple, KEYNAME_LEN, "SPACE");
    snprintf(s->jump, KEYNAME_LEN, "X");
...
static void SetField(Settings *s, const char *key, const char *value) {
    ...
    else if (strcmp(key, "grapple") == 0) snprintf(s->grapple, KEYNAME_LEN, "%s", value);
    else if (strcmp(key, "jump") == 0) snprintf(s->jump, KEYNAME_LEN, "%s", value);
...
bool Settings_Save(const Settings *s) {
    ...
    fprintf(f,
        ...
        s->grapple, s->jump, s->pause, s->restart, s->quit,
        s->volumePercent, s->audioEnabled ? 1 : 0, s->scanlines ? 1 : 0);
