...
static bool GetAppDir(char *out, size_t outSize, const char *kind) {
    ...
    /* mkdir -p for base (it usually already exists) and base/skyraid. */
    char cursor[600] = {0};
...
void Settings_Default(Settings *s) {
    ...
    snprintf(s->fire2, KEYNAME_LEN, "W");
...
static void SetField(Settings *s, const char *key, const char *value) {
    ...
    else if (strcmp(key, "fire2") == 0) snprintf(s->fire2, KEYNAME_LEN, "%s", value);
...
bool Settings_Save(const Settings *s) {
    ...
    fprintf(f,
        ...
        "audio_enabled=%d\n"
        "scanlines=%d\n",
        s->left, s->left2, s->right, s->right2, s->fire, s->fire2,
